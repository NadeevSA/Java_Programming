...
public class Main {
    ...
    private static ArrayList<ArrayList<int[]>> getMaxLinesByDirection(ArrayList<int[]> moves, int[] vector) {
        ...
        if (moves.isEmpty())
            return null;
        char sym = field[moves.get(0)[Y]][moves.get(0)[X]];
        int dx = vector[X];
        int dy = vector[Y];
    ...
    private static int[] getBestMove(ArrayList<int[]> moves) {
        ...
        y = line.get(line.size() - 1)[Y] + line.get(line.size() - 1)[DY];
        x = line.get(line.size() - 1)[X] + line.get(line.size() - 1)[DX];
        if (!isEmptyCell(y, x)) {
            y = line.get(0)[Y] - line.get(0)[DY];
            x = line.get(0)[X] - line.get(0)[DX];
        }
