...
public class Main {
    ...
    static final int SIZE_Y = 8; //размер поля по вертикале
    static final int SIZE_X = 8; //расчет поля по горизонтале
    ...
    private static float scoring(int x, int y, char sign){

        float count = 0;

        count += checkDiaUp(y,x,sign);
        count += (checkDiaUp(y,x,EMPTY_DOT)/2);

        count += checkDiaDown(y,x,sign);
        count += (checkDiaDown(y,x,EMPTY_DOT)/2);

        count += checkLineHorisont(y,x,sign);
        count += (checkLineHorisont(y,x,EMPTY_DOT)/2);

        count += checkLineVertical(y,x,sign);
        count += (checkLineVertical(y,x,EMPTY_DOT)/2);
        return count;
    }
    private static void AiMove1(){
        int x, y;
        do {
            y = rnd.nextInt(SIZE_Y);
            x = rnd.nextInt(SIZE_X);
        ...
        float count = 0;
        //блокировка
        for(int i = SIZE_WIN; i < SIZE_Y-SIZE_WIN; i++){
            for(int j = SIZE_WIN; j < SIZE_X-SIZE_WIN; j++){
                float score_b = scoring(i,j,player_DOT);
                float score_w = scoring(i,j,Ai_DOT);
                if(score_w > count){
                    count = score_w;
                    x = j;
                    y = i;
